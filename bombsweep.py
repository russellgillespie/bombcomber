...
class MyGame(object):
	def __init__(self, width, height, boardsize=1, minedensity=2):
		...
		self.tileh = int((self.height - self.hud_height)/self.tile_size)
...
#		self.background=pygame.image.load("background.png")
		...
		self.tile=pygame.transform.scale(self.tile, (int(self.tile_size), int(self.tile_size)))
		self.tiledown=pygame.transform.scale(self.tiledown, (int(self.tile_size), int(self.tile_size)))
#		self.background=pygame.transform.scale(self.background, (int(self.tile_size), int(self.tile_size)))
		self.mine=pygame.transform.scale(self.mine, (int(self.tile_size), int(self.tile_size)))
		self.flag=pygame.transform.scale(self.flag, (int(self.tile_size), int(self.tile_size)))
		self.num1=pygame.transform.scale(self.num1, (int(self.tile_size), int(self.tile_size)))
		self.num2=pygame.transform.scale(self.num2, (int(self.tile_size), int(self.tile_size)))
		self.num3=pygame.transform.scale(self.num3, (int(self.tile_size), int(self.tile_size)))
		self.num4=pygame.transform.scale(self.num4, (int(self.tile_size), int(self.tile_size)))
		self.num5=pygame.transform.scale(self.num5, (int(self.tile_size), int(self.tile_size)))
		self.num6=pygame.transform.scale(self.num6, (int(self.tile_size), int(self.tile_size)))
		self.num7=pygame.transform.scale(self.num7, (int(self.tile_size), int(self.tile_size)))
		self.num8=pygame.transform.scale(self.num8, (int(self.tile_size), int(self.tile_size)))
		self.num9=pygame.transform.scale(self.num9, (int(self.tile_size), int(self.tile_size)))
		self.num0=pygame.transform.scale(self.num0, (int(self.tile_size), int(self.tile_size)))
	...
	def initBG(self):
		self.bg = {}
		self.bg = [[False for y in range(self.tileh)] for x in range(self.tilew)]
...
# 				self.bg[x][y] = self.background
	...
	def initMines(self):
		self.mines = {}
		self.mines = [[False for y in range (self.tileh)] for x in range(self.tilew)]
	...
	def initVoidCheck(self):
		self.voidCheck = {}
		self.voidCheck = [[False for y in range(self.tileh)] for x in range(self.tilew)]
	
	def initMap(self):
		#initialize tilemap dictionary
		self.map = {}
		self.map = [[False for y in range(self.tileh)] for x in range(self.tilew)]
	...
	def drawBG(self):
		for x in range(self.tilew):
			for y in range(self.tileh):		
				self.screen.blit(self.numMines[self.bg[x][y]], (x*self.tile_size, y*self.tile_size))
	...
	def drawHUD(self):
		...
		#create text surface
		time_display = self.fixed48.render("%02d:%02d" % (m, s), 1, (50,50,50))
		score = self.myfont64.render("Score:%02d" % self.score, 1, (244,249,253))
		mines_left = self.myfont64.render("Bombs:%d" % self.mines_left, 1, (244,249,253))

		#draw surface
		time_align = self.width/2 - time_display.get_width()/2
		self.screen.blit(time_display, (time_align, hud_offset - time_display.get_height()//2))
		self.screen.blit(score, (20, hud_offset - score.get_height()//2))
		self.screen.blit(mines_left, (self.width - mines_left.get_width() - 20, hud_offset - mines_left.get_height()//2))

	def drawMines(self):
		for x in range(self.tilew):
			for y in range(self.tileh):
				if self.mines[x][y] != False:
					self.screen.blit(self.mines[x][y], (x*self.tile_size, y*self.tile_size))
			...
			for y in range(self.tileh):
				try:
					if self.map[x][y] != False:
						self.screen.blit(self.map[x][y], (x*self.tile_size, y*self.tile_size))
				except:
					self.screen.blit(self.flag, (x*self.tile_size, y*self.tile_size))
	...
	def countMines(self, xpos, ypos):
		...
		for x, y in self.matrix:
			try:
				if self.mines[xpos+x][ypos+y] != False and xpos < self.tilew and xpos+x >= 0 and ypos < self.tileh and ypos+y >=0:
					self.count += 1
			except:
				pass
		return self.count
	...
	def fillVoid(self, xpos, ypos):
		...
		for x, y in self.matrix:
			try:
				if xpos+x >= 0 and ypos+y >= 0: self.map[xpos+x][ypos+y] = False
				if self.bg[xpos+x][ypos+y] == 0 and self.voidCheck[xpos+x][ypos+y] == False and xpos+x >= 0 and ypos+y >= 0:
...
#		for time in range(2):
	...
	def voidConnect(self):
		for row in range(self.tilew):
			for col in range(self.tileh):
				for x, y in self.matrix:
					try:
						if self.voidCheck[row+x][col+y] == True and row+x >=0 and col+y >= 0:
	...
	def gamewin(self):
		if self.minescore == self.score:
			...
			if map_clear + self.score == self.tilew * self.tileh:
				...
				iwin = self.myfont96.render("You're the Bomb!", 1, (0,0,0))
				self.screen.blit(iwin, (self.width/2 - iwin.get_width()/2, self.height/2 - iwin.get_height()/2))
				pygame.display.flip()
	...
	def update(self):
		...
		if event.type == pygame.QUIT:
			exit()
		if event.type == pygame.MOUSEBUTTONDOWN and self.map[xpos][ypos] != False: match.play()	
		if mouse_pressed[0] == True and self.map[xpos][ypos] != False:
			self.screen.blit(self.tiledown, (xpos * self.tile_size, ypos * self.tile_size))
		try:
...
def menu():
	...
	while 1:
		selected = [(244,249,253),(244,249,253),(244,249,253),(244,249,253)]
		...
		try:
			selected[selector] = (238,143,41)
		except IndexError:
			pass
			
		try:
			if to_click != selected: menu_click.play()
		except:
			pass
		...
		menu_surface.blit(easy,  (width//2-easy.get_width()//2, height*3//8))
		menu_surface.blit(normal,  (width//2-normal.get_width()//2, height*4//8))
		menu_surface.blit(hard,  (width//2-hard.get_width()//2, height*5//8))
		menu_surface.blit(expert,  (width//2-expert.get_width()//2, height*6//8))	
		...
		if event.type == pygame.MOUSEBUTTONUP:
			try:
				splash_loop.stop()
				fuse.play()
				boardsize, minedensity = difficulty[selector]
...
def new_game(width, height, boardsize, minedensity):
